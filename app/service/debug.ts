...
class DebugService {
  ...
  constructor() {
    if (PerformanceObserver) {
      this.longtask = new PerformanceObserver(list =>
        list.getEntries().forEach(this.displayLongTask)
      );
      this.longtask.observe({ entryTypes: ['longtask'] });

      this.paintPerformance = new PerformanceObserver(list =>
        list.getEntries().forEach(this.displayPaintInfo)
      );
  ...
  private displayLongTask(entry) {
    console.log(
      `Long Task detected. Execution time:`,
